...
class SelectValidators():
    def __init__(self, _network):
        ...
        self.my_subscan_api_key = '2cd21775dd54478782ab6e20df25ef17'  # 我的 subscan api 密钥
        self.max_row = 100  # subscan api 一页最多有多少行（row）
        self.rows = []      # 验证人总信息表
        self.df_validators = pd.DataFrame()

        self.filepath_validators_list = 'C://Users//梦龙//Desktop//波卡视频//999 验证人评估2025.7.9//新方法//' + self.network +'_validators.xlsx'
                                        
        if self.network == 'polkadot':
            self.current_era = 1853              # 波卡 当前时代
            self.average_stake = 1.3935 * 10**6  # 当前的全网平均质押量
            self.validator_counts = 600          # 当前时代的验证人总数
            self.coefficient = 1.2               # 验证人的质押量不能超过平均质押量的多少
            self.fee_upper_limit = 0.1           # 验证人手续费最高是多少
            self.page_counts = 6                 # 一页最多100条数据，总共需要多少数据（波卡是6页，ksm是10页）
            self.days = 50                       # 验证人在过去50天连续出块，如果去除当前时代，是49天
        elif self.network == 'kusama':
            self.current_era = 8260        # Kusama 当前时代
            self.average_stake = 7845      # 当前的全网平均质押量
            self.validator_counts = 1000   # 当前时代的验证人总数
            self.coefficient = 1.2         # 验证人的质押量不能超过平均质押量的多少
            self.fee_upper_limit = 0.15     # 验证人手续费最高是多少
    ...
    def get_one_page(self, _page):
        ...
        # 在收到的数据中获取几个关键字段（地址，手续费，总质押量,单个验证人的提名人数量）
        for item in response_dict['data']['list']:
            row = {
                ...
                'count_nominators': item['count_nominators']
            }
    ...
    def get_validators_list(self):
        for i in range(self.page_counts):
            ...
            self.get_one_page(i)
        self.df_validators = pd.DataFrame(self.rows, columns=['stash_account_display', 'validator_prefs_value', 'bonded_total','count_nominators']) # 使用pandas创建数据框
    ...
    def produce_block_continuously_all(self, _df_validators):
        ...
        for i in range(continue_number, _df_validators.shape[0]):
            ...
            print("当前时代{0}  最早时代{1}".format(self.current_era, long_ago_era))
            # time.sleep(1)
            # 如果此条件成立，表示验证人连续出块（用 1 表示）。否则（两个时代差值过大）表示验证人在过去 days 天没有连续出块（用 0 表示）
            # 对 kusama 的条件放宽
            if self.network == 'polkadot':
                produce_block_continuously = ((self.current_era - long_ago_era) >= (self.days-1) and (self.current_era - long_ago_era) <= (self.days+1))
            elif self.network == 'kusama':
                produce_block_continuously = ((self.current_era - long_ago_era) >= (self.days-2) and (self.current_era - long_ago_era) <= (self.days+2))
            else:
                print('网络名称出错！')
                time.sleep(30)

            if produce_block_continuously:
            ...
            print("第{0}个验证人的地址是{1}，是否在过去连续出块{2}".format(i, address, produce_block_continuously))
    ...
    def modify_low_fee_validators(self, _df_validators):
        ...
        _df_validators['mean_modify'] = _df_validators['mean']*(1-_df_validators['fee'])/(_df_validators['bonded_total']/self.average_stake)
...
# 步骤：
# network = 'polkadot'  # 设置验证人网络（polkadot/kusama）
...
# # 4：运行以下程序，获取所有低手续费验证人的原始数据（平均时代得分）如果程序中断，则自动保存序号，再次执行，从序号处重新开始获取数据
# # #                  对低手续费验证人的原始平均时代得分进行调整，并画图
...
# # # 5：得到最终表格后，对是否连续出块列进行筛选（produce_block_continuously），将连续出块的验证人（1）标红
# # #   最后加2列（名称，验证人总数）
# # #   按照从高到低的顺序选择3个验证人，在 波卡js钱包 查询验证人的名称及其公司控制的验证人总数
# # #   确认其没有惩罚记录 ，最后将确定选择的验证人底色涂黄（波卡：3  Kusama：5），更换验证人
# # #   只有在每个月的 9 号才需要把质押收益再质押进去，19 号和 29 号不要再质押
# # #   发 X
...
# time_day = 31 # 质押了多少天
# initial_stake = 6.8384  # 初始质押量
# reward = 0.0792   # 一个月的质押奖励

# final_stake = initial_stake + reward  # 结束质押量

# r_month = reward / initial_stake # 一个月的收益率
# APR = r_month / time_day * 365 # 不复投情况下的APR
# APY = (1+r_month)**(365/time_day) - 1 # 复投情况下的APY

# print('结束质押量为{0}  收益率为{1}  APR为{2}  APY为{3}'.format(final_stake, r_month, APR, APY))
